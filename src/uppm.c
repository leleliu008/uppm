...
int uppm_main(int argc, char* argv[]) {
    ...
    if (strcmp(argv[1], "info") == 0 || strcmp(argv[1], "info-available") == 0 || strcmp(argv[1], "info-installed") == 0) {
        ...
        if (ret == UPPM_ERROR_ARG_IS_NULL) {
            ...
            fprintf(stderr, "uppm package '%s' is not installed.\n", argv[2]);
        } else if (ret == UPPM_ERROR_ENV_HOME_NOT_SET) {
            fprintf(stderr, "%s\n", "HOME environment variable is not set.\n");
        } else if (ret == UPPM_ERROR_ENV_PATH_NOT_SET) {
            fprintf(stderr, "%s\n", "PATH environment variable is not set.\n");
    ...
    if (strcmp(argv[1], "install") == 0) {
        if (argv[2] == NULL) {
            LOG_ERROR1("uppm package name is unspecified.");
            return UPPM_ERROR;
        }
    ...
    if (strcmp(argv[1], "upgrade") == 0) {
        ...
        for (int i = 2; i < argc; i++) {
            if (strcmp(argv[i], "-v") == 0) {
                ...
                if (ret == UPPM_OK) {
                    ...
                    LOG_ERROR3("uppm package '", argv[i], "' is not outdated.");
                }
        ...
        for (int i = 0; i < packageNameIndexArraySize; i++) {
            ...
            if (ret == UPPM_ERROR_ARG_IS_NULL) {
                ...
                fprintf(stderr, "uppm package '%s' is not outdated.\n", packageName);
            } else if (ret == UPPM_ERROR_ENV_HOME_NOT_SET) {
                fprintf(stderr, "%s\n", "HOME environment variable is not set.\n");
            } else if (ret == UPPM_ERROR_ENV_PATH_NOT_SET) {
                fprintf(stderr, "%s\n", "PATH environment variable is not set.\n");
